...
#    Copyright (C) 2014 - Today Riza and Team
...
#        'hotel_id': fields.many2one('hotel.hotel', required=True),
    ...
    def _create_folio(self, cr, uid, ids, context=None):
        ...
        hotel_folio_line_obj = self.pool.get('hotel.folio.line')
        for reservation in self.browse(cr, uid, ids, context=context):
            folio_lines = []
            ...
            for line in reservation.reservation_line:
                for r in line.reserve:
                    res = hotel_folio_line_obj.product_id_change(cr, uid, [], reservation.pricelist_id.id, r.product_id.id, qty=0,
                                      uom=r['uom_id'].id, qty_uos=0, uos=False, name='', partner_id=reservation.partner_id.id,
...
#                         'price_unit': r['lst_price'],
                        'price_unit': res.get('value') and res.get('value').get('price_unit') or 0.0,
                    ...
                    line_temp = room_obj.write(cr, uid, [r.id], {'status': 'occupied'}, context=context)
